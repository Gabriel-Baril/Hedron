...
namespace dm
{
	...
	{
		DM_CORE_ASSERT(!m_MetricStack.empty(), "Empty stack!");
	...
	{
		DM_INFO_LOG("Metric Entry Count: {0}", m_StoredMetrics.size());
		for (int i = 0; i < m_StoredMetrics.size(); i++)
		{
			...
			DM_INFO_LOG("Operation({0}), ObjectID({1}), Time({2}ms)", object_operation_type_to_string(current.opType), current.objectID, timeMS);
		}
	}
}
