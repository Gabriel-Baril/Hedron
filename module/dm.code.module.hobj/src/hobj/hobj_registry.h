...
namespace dm
{
	...
	private:
		...
		T *object_get(uuid64 id)
		{
			...
			{
				DM_WARNING_LOG("Object with id '{0}' was not found in the manifest!", id);
				return nullptr;
			}
		...
		T *object_get(const char *name)
		{
			...
			{
				DM_WARNING_LOG("Object with name '{0}' was not found in the manifest!", name);
				return nullptr;
			}
		...
		{
			DM_CORE_ASSERT(!m_Sources.contains(sourceName), "Cannot add 2 sources with the same name!");
