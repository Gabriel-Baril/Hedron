...
namespace dm
{
	...
	{
		DM_CORE_ASSERT(ptr >= (u8 *)allocator.memory, "Out of bound pointer!");
		DM_CORE_ASSERT(ptr < (u8 *)allocator.memory + allocator.totalSize, "Out of bound free on linear allocator (outside bounds)");
		DM_CORE_ASSERT(ptr < (u8 *)allocator.memory + allocator.allocatedSize, "Out of bound free on linear allocator (inside bounds, after allocated)");
