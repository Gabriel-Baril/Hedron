...
namespace dm
{
	...
	{
		DM_WARNING_LOG("Received signal: {}", signal);
	...
	BOOL WINAPI console_handler_callback(DWORD signal)
	{
		...
		{
			DM_WARNING_LOG("Console is closing or interrupted!");
	...
	void log_init()
	{
		...
		try
		{
...
#if USING(DM_LOG_CONSOLE_ENABLE)
			...
			s_CoreLogger = make_ref<spdlog::logger>("DM", sinks.begin(), sinks.end());
			s_CoreLogger->set_level(spdlog::level::trace);

			spdlog::set_default_logger(s_CoreLogger);
			spdlog::set_level(spdlog::level::trace);
#if USING(DM_LOG_ALWAYS_FLUSH)
