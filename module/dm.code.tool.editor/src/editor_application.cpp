...
namespace dm
{
	...
	void EditorApplication::on_event(Event &event)
	{
		...
		dispatcher.dispatch<WindowResizedEvent>(DM_BIND_EVENT_FN(EditorApplication::on_window_resized));
		dispatcher.dispatch<KeyPressedEvent>(DM_BIND_EVENT_FN(EditorApplication::on_key_pressed));
		dispatcher.dispatch<MouseButtonPressedEvent>(DM_BIND_EVENT_FN(EditorApplication::on_mouse_button_pressed));
	}
	...
	{
		// DM_INFO_LOG("EditorApplication::OnKeyPressed - Pressed!");
		return false;
	}
	...
	EditorApplication::EditorApplication()
	{
		...
		m_Window.SetEventCallback(DM_BIND_EVENT_FN(EditorApplication::on_event));
	...
	{
		Ref<VulkanModel> dmModel = VulkanModel::create_model_from_obj_file(&m_Device, get_data_path("models/flat_vase.obj"));
		...
		{
			dmModel = VulkanModel::create_model_from_obj_file(&m_Device, get_data_path("models/quad.obj"));
		...
		{
			dmModel = VulkanModel::create_model_from_fbx_file(&m_Device, get_data_path("models/cube.fbx")); // models/cube.fbx
