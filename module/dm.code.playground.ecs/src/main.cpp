...
json serialize_entity(flecs::entity e)
{
	...
	j["name"] = e.name();
	if (e.has<Position>())
		j["position"] = serialize_component(*e.get<Position>());
	if (e.has<Health>())
		j["health"] = serialize_component(*e.get<Health>());
	if (e.has<Damage>())
		j["damage"] = serialize_component(*e.get<Damage>());
...
flecs::entity deserialize_entity(flecs::world &ecs, const json &j, flecs::entity parent = flecs::entity())
{
	...
	if (j.contains("position"))
		e.set(deserialize_position(j["position"]));
	if (j.contains("health"))
		e.set(deserialize_health(j["health"]));
	if (j.contains("damage"))
		e.set(deserialize_damage(j["damage"]));
