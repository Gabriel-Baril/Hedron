...
namespace dm
{
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL debug_callback(
			...
			void *pUserData)
	{
		DM_MAYBE_UNUSED(messageSeverity);
		DM_MAYBE_UNUSED(messageType);
		DM_MAYBE_UNUSED(pUserData);

		DM_ERROR_LOG("Validation Layer: {0}", pCallbackData->pMessage);
		return VK_FALSE;
	}
	...
	void VulkanDevice::create_instance()
	{
		...
		{
			DM_CORE_THROW(std::runtime_error, "validation layers requested, but not available!");
		}
		...
		{
			DM_CORE_THROW(std::runtime_error, "failed to create instance!");
		}
	...
	void VulkanDevice::pick_physical_device()
	{
		...
		if (deviceCount == 0)
		{
			DM_CORE_THROW(std::runtime_error, "failed to find GPUs with Vulkan support!");
		}
		DM_INFO_LOG("Device Count: {0}", deviceCount);
		...
		{
			DM_CORE_THROW(std::runtime_error, "failed to find a suitable GPU!");
		}

		vkGetPhysicalDeviceProperties(m_PhysicalDevice, &m_Properties);
		DM_INFO_LOG("Physical Device: {0}", deviceCount);
	}
	...
	void VulkanDevice::create_logical_device()
	{
		...
		{
			DM_CORE_THROW(std::runtime_error, "failed to create logical device!");
		}
	...
	void VulkanDevice::create_command_pool()
	{
		...
		{
			DM_CORE_THROW(std::runtime_error, "failed to create command pool!");
		}
	}
	...
	void VulkanDevice::setup_debug_messenger()
	{
		...
		{
			DM_CORE_THROW(std::runtime_error, "failed to set up debug messenger!");
		}
	}
	...
	void VulkanDevice::has_gflw_required_instance_extensions()
	{
		...
		DM_INFO_LOG("Available Extensions:");
		unordered_set<string> available;
		for (const auto &extension : extensions)
		{
			DM_INFO_LOG("\t{0}", extension.extensionName);
			available.insert(extension.extensionName);
		}

		DM_INFO_LOG("Required Extensions:");
		auto requiredExtensions = get_required_extensions();
		for (const auto &required : requiredExtensions)
		{
			DM_INFO_LOG("\t{0}", required);
			if (available.find(required) == available.end())
			{
				DM_CORE_THROW(std::runtime_error, "Missing required glfw extension");
			}
		}
	}
			...
			const vector<VkFormat> &candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
	{
		...
		DM_CORE_THROW(std::runtime_error, "failed to find supported format!");
	...
	uint32_t VulkanDevice::find_memory_type(uint32_t typeFilter, VkMemoryPropertyFlags properties)
	{
		...
		DM_CORE_THROW(std::runtime_error, "failed to find suitable memory type!");
			...
			VkDeviceMemory &bufferMemory)
	{
		...
		{
			DM_CORE_THROW(std::runtime_error, "failed to create vertex buffer!");
		}
		...
		{
			DM_CORE_THROW(std::runtime_error, "failed to allocate vertex buffer memory!");
		}
			...
			VkDeviceMemory &imageMemory)
	{
		...
		{
			DM_CORE_THROW(std::runtime_error, "failed to create image!");
		}
		...
		{
			DM_CORE_THROW(std::runtime_error, "failed to allocate image memory!");
		}

		if (vkBindImageMemory(m_Device, image, imageMemory, 0) != VK_SUCCESS)
		{
			DM_CORE_THROW(std::runtime_error, "failed to bind image memory!");
		}
	}
